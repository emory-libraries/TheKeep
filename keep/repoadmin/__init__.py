...
(targeted for staff members and data curators)
...
2. Create a new list-view template snippet (it is recommend to extend
   ``repoadmin/snippets/list-item.html``) in the app where the object
   is defined.
3. Add the new type to the list of ``known_object_types`` in
   :class:`~keep.repoadmin.views.keyword_search` and update the search
