...
class SourceTechForm(XmlObjectForm):
    ...
    """
    note = forms.CharField(label='General Note', widget=forms.Textarea,
        help_text='General note about the physical item', required=False)
    related_files = forms.CharField(required=False,
        widget=forms.TextInput(attrs={'class': 'long'}),
        help_text='IDs of other digitized files for the same item, separated by semicolons')
        # according to spec, related_files is required if multi-part - not tracking for Min Items
    manufacturer = forms.CharField(label='Tape: Manufacturer', required=False,
       help_text='The manufacturer of the magnetic tape')
    stock = forms.CharField(label='Tape: Brand/Stock',
       help_text='The brand or stock of the magnetic tape')
    # TODO/FIXME: form is not required - do we have to explicitly specify a
    # field just to set help text & not required?
    ...
    def __init__(self, initial={}, **kwargs):       
        super(SourceTechForm, self).__init__(initial=initial, **kwargs)
        ...
        if 'reel' not in self.initial and 'reel_size-value' in self.initial:
            initial['reel'] = self.initial['reel_size-value']
...
class AudioObjectEditForm(forms.Form):
    ...
    # these are really just place-holders for now
    mods = ModsEditForm()
