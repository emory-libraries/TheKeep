...
A basic build deploy script for the Keep, using Fabric 1.1 or
greater.
...
Also includes task to build the local version (install dependencies,
...
@task
def check_sysdeps():
    '''Check non-Python dependencies on the remote server.

    Use dpkg to check that expected packages are available.  Assumes
    a dpkg/apt-get system (Ubuntu/Debian).
    '''
    not_installed = []
    ...
    if not_installed:
        puts(red('The following packages are not installed:\n%s' % \
                    ', '.join(not_installed)))
...
def configure_site():
    ...
    # TODO: re-enable once app has been switched over to staticfiles
    # with cd('%(remote_path)s/%(build_dir)s' % env):
    #     with prefix('source env/bin/activate'):
...
def deploy(path=None, user=None, url_prefix='', check_svn_head=True, python=None,
    ...
    Parameters:
      path: base deploy directory on remote host; deploy expects a
            ...
            Default: env.remote_path = /home/httpd/sites/keep
    ...
    Example usage:
      ...
      fab deploy:user=www-data,url_prefix=/keep -H servername
